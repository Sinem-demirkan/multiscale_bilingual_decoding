...
def load_subject_data(subject: str, masker, args):
    ...
    for run in runs:
        ...
        for idx in keep_idx:
            ...
            trial_counter += 1
            event_row = events.iloc[int(idx)]
            trial_rows.append(
                {
                    "subject": subject,
                    "trial_index": trial_counter,
                    "run": int(run),
                    "trial_in_run": int(idx + 1),
                    "trial_type": event_row.get(args.trial_column, np.nan),
                    "stimulus": event_row.get("stimulus", np.nan),
                    "language": y_full[int(idx)],
                    "true_y": 1 if y_full[int(idx)] == "L1" else 0,
                }
            )
    ...
    return X, y, groups, pd.DataFrame(trial_rows)
...
def main():
    ...
    for sub in range(args.start_sub, args.end_sub + 1):
        ...
        X, y, groups, trial_meta = load_subject_data(subject, masker, args)
        ...
        parcel_cols = [f"parcel_{i:03d}" for i in range(1, X.shape[1] + 1)]
        subject_features = pd.DataFrame(X, columns=parcel_cols)
        subject_features.insert(0, "trial_index", trial_meta["trial_index"].to_numpy())
        subject_features.insert(0, "subject", subject)
        feature_rows.append(subject_features)
        label_rows.append(trial_meta)
    ...
    features.to_csv(args.out_dir / "whole_cortex_trialwise_parcel_features.csv", index=False)
    labels.to_csv(args.out_dir / "whole_cortex_trialwise_labels.csv", index=False)
